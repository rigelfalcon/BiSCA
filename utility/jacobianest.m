function [jac, err] = jacobianest(fun, x0, prescreen) %
    ...
    if nargin < 3 || isempty(prescreen)
        prescreen = true;
    end

    if prescreen %differentiable
        epsilon = 1e-3;
        tolerance = 1e-8; %y0 = fun(x0);tolerance =1e-6* min(y0(y0 ~= 0))
        isActive = ~find_inactive_variables(fun, x0, epsilon, tolerance);
    else
        isActive = true(1, nx);
    end
    ...
    relativedelta = MaxStep * StepRatio .^ (0:-1: - (MaxScale - 1));
    ...
    %
    % get the romberg points
    nexpon = length(rombexpon);
    ne = nsteps;
    [i, j] = ndgrid(1:nexpon + 2, 0:ne - (nexpon + 2) - 1);
    idx_rhs = i + j;

    romberg = init_romberg(StepRatio, rombexpon);
    ...
    for i = find(isActive)
        ...
        for j = 1:n
            ...
            [der_romb, errest] = rombextrap(romberg, derest(j, :), rombexpon, idx_rhs);
...
function [der_romb, errest] = rombextrap(romberg, der_init, rombexpon, idx_rhs)
    rmat = romberg.rmat;
    rromb = romberg.rromb;
    qromb = romberg.qromb;
    nexpon = length(rombexpon);

    rhs = der_init(idx_rhs);
...
function romberg = init_romberg(StepRatio, rombexpon)
    srinv = 1 / StepRatio;
...
function [der_romb, errest] = rombextrap_full(StepRatio, der_init, rombexpon, idx_rhs)
